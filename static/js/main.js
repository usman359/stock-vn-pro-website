...
if (!fetchDataBtn || !tickerSelect || !startDateInput || !endDateInput || !modelSelect) {
...
function setupEventListeners() {
    ...
    tickerSelect.addEventListener('change', function() {
        ...
        const selectedCompany = tickerSelect.options[tickerSelect.selectedIndex].text;
...
function updatePlotsTheme(theme) {
    const plotIds = ['stock-plot', 'trend-plot', 'seasonal-plot', 'residual-plot', 'prediction-plot', 'components-plot'];
...
async function fetchStockData() {
    ...
    if (isDataFetching) {
        console.log("A data fetch operation is already in progress. Skipping this request.");
        return;
    }
    ...
    try {
        ...
        ['stock-plot', 'trend-plot', 'seasonal-plot', 'residual-plot', 'prediction-plot', 'components-plot'].forEach(plotId => {
        ...
        if (tickerSelect && tickerSelect.selectedIndex >= 0) {
            ...
            console.log(`Fetching data for ${currentCompany} (${tickerSelect.value})`);
        } else {
        ...
        if (!tickerValue) {
            throw new Error("No ticker symbol selected. Please select a company or enter a custom ticker.");
        }
        ...
        if (!response.ok) {
            ...
            if (response.status === 404) {
                        ...
                        <div class="modal-dialog modal-dialog-centered">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p>We couldn't find data for <strong>${currentCompany || tickerValue}</strong> in our data sources.</p>
                ...
                if (!document.getElementById('dataErrorModal')) {
                    ...
                    document.getElementById('tryDifferentTicker').addEventListener('click', function() {
                ...
                // Show the modal
                const errorModal = new bootstrap.Modal(document.getElementById('dataErrorModal'));
                errorModal.show();
                
                throw new Error(`No data available for ${currentCompany || tickerValue}. Please try a different company or ticker symbol.`);
            }
        ...
        if (!result.data || !Array.isArray(result.data) || result.data.length === 0) {
                ...
                <div class="modal fade" id="emptyDataModal" tabindex="-1" aria-hidden="true">
                    <div class="modal-dialog modal-dialog-centered">
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                <p>We received a response from our data provider, but no data was available for <strong>${currentCompany || tickerValue}</strong> in the selected date range.</p>
                                <p>You can try:</p>
                                <ul>
            ...
            if (!document.getElementById('emptyDataModal')) {
                ...
                document.getElementById('adjustDateRange').addEventListener('click', function() {
            ...
            // Show the modal
            const emptyDataModal = new bootstrap.Modal(document.getElementById('emptyDataModal'));
            emptyDataModal.show();
            
            throw new Error("No data available for this company. Please try a different ticker or date range.");
        }
        
        console.log(`Successfully fetched data for ${currentCompany || tickerValue}: ${result.data.length} records from ${result.source}`);
        ...
        if (!document.getElementById('dataErrorModal')?.classList.contains('show') && 
...
function updateCompanyDisplay(companyName) {
    ...
    const companyInfoElements = document.querySelectorAll('.current-company-info');
...
function displayStockData(data, columns, source = 'Stooq') {
    ...
    if (!data || data.length === 0 || !columns || columns.length === 0) {
        ...
        showErrorMessage('Error: No data available to display. Please try with a different ticker or date range.');
        return;
    }
    ...
    if (!tableHeader || !tableBody || !dataInfo) {
        ...
        showErrorMessage('Error: Some table elements not found. Please refresh the page and try again.');
        return;
    }
    ...
    try {
        ...
        if (!tableResponsive) {
            ...
            showErrorMessage('Error: Could not find table container. Please refresh the page and try again.');
            return;
        }
        ...
        if (!newTableHeader || !newTableBody) {
            ...
            showErrorMessage('Error: Failed to create table. Please refresh the page and try again.');
            return;
        }
            ...
            toggleAllDataBtn.addEventListener('click', function() {
                ...
                if (dataTable) {
                    if (dataTable.style.maxHeight === 'none') {
                        ...
                        this.innerHTML = '<i class="fas fa-compress-alt me-1"></i>Show Less';
                    }
                }
            });
        }
        ...
        const priceColumnIndex = columns.indexOf('Close') !== -1 ? columns.indexOf('Close') : 
                                columns.indexOf('Adj Close') !== -1 ? columns.indexOf('Adj Close') : -1;
        ...
        data.forEach((row, rowIndex) => {
            ...
            columns.forEach((column, colIndex) => {
                ...
                if (column === 'Date') {
                    ...
                    const icon = change >= 0 ? '<i class="fas fa-caret-up me-1"></i>' : '<i class="fas fa-caret-down me-1"></i>';
...
function createStockPlot(data, columns) {
    ...
    try {
        ...
        if (!data[0].Date || !data[0].Close) {
            ...
            showErrorMessage("Error creating stock plot: Missing required data columns");
            return;
        }
        ...
        const hasVolume = columns.includes('Volume') && data[0].Volume !== undefined;
...
async function checkStationarity() {
    ...
    try {
        ...
        if (result.is_stationary) {
            stationarityResult.textContent = 'Yes, the data is stationary (p-value < 0.05)';
            stationarityResult.className = 'alert alert-success';
        } else {
            stationarityResult.textContent = 'No, the data is not stationary (p-value >= 0.05)';
...
async function createDecomposition() {
    ...
    showSectionLoading('decomposition-section', 'Creating time series decomposition...');
...
async function runSelectedModel() {
    if (!selectedColumn || stockData.length === 0) {
        ...
        showErrorMessage('Error: No data available or column not selected. Please fetch data first.');
        return;
    }
    ...
    if (!modelSection) {
        ...
        showErrorMessage('Error: Model section not found. Please refresh the page and try again.');
        return;
    }
    
    console.log(`Running ${modelType} model for ${selectedColumn} with ${stockData.length} data points`);
    ...
    if (!metricsSection || !componentsPlot) {
        ...
        showErrorMessage('Error: UI elements not found. Please refresh the page and try again.');
        return;
    }
    ...
    try {
        ...
        switch (modelType) {
            case 'Transformer':
                ...
                console.log(`Transformer params: seq_len=${sequence_length}, head_size=${head_size}, num_heads=${num_heads}`);
        ...
        console.log(`Sending request to ${endpoint} with payload for ${selectedColumn}`);
        ...
        if (!response.ok) {
            ...
            throw new Error(`Failed to run ${modelType} model: ${response.status} ${response.statusText}`);
        }
        ...
        setTimeout(async () => {
            try {
                ...
                showErrorMessage(`Error processing ${modelType} model response: ${parseError.message}`);
                hideModelLoading();
            }
        }, 800);
...
function displayModelResults(result, modelType) {
    ...
    if (!result || !result.metrics || !result.predictions || !result.future_predictions) {
        console.error('Invalid model results', result);
        showErrorMessage(`Error: Received invalid ${modelType} model results from server`);
        return;
    }
    ...
    const lastPrice = stockData.map(row => row[selectedColumn])[stockData.length - 1];
    const futurePredictions = result.future_predictions;
    const changePercents = futurePredictions.map(val => ((val - lastPrice) / lastPrice * 100).toFixed(2));
    ...
    annotations.push({
        ...
        xref: 'x',
        yref: 'y',
        text: `<b>3-Day Forecast Summary</b><br>` +
    ...
    try {
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${futurePredictions[0].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[0] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[0] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
                ...
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${futurePredictions[1].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[1] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[1] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
                ...
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${futurePredictions[2].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[2] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[2] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
        ...
        if (predictionPlot.nextElementSibling) {
            predictionPlot.parentNode.insertBefore(predictionInfo, predictionPlot.nextElementSibling);
        } else {
...
function displayProphetResults(result) {
    ...
    if (!result || !result.forecast || !result.components) {
        ...
        showErrorMessage('Error: Received invalid Prophet model results from server');
        return;
    }
    ...
    try {
        ...
        const changePercents = threeDayForecast.values.map(val => ((val - lastPrice) / lastPrice * 100).toFixed(2));
            ...
            <div class="row">
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${threeDayForecast.values[0].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[0] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[0] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
                            ${Math.abs(changePercents[0])}%
                        </p>
                        <p class="small">
                            ...
                            $${threeDayForecast.lower[0].toFixed(2)} - $${threeDayForecast.upper[0].toFixed(2)}
                        </p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${threeDayForecast.values[1].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[1] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[1] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
                            ${Math.abs(changePercents[1])}%
                        </p>
                        <p class="small">
                            <span class="text-muted">Range:</span> 
                            $${threeDayForecast.lower[1].toFixed(2)} - $${threeDayForecast.upper[1].toFixed(2)}
                        </p>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="prediction-day">
                        ...
                        <p class="prediction-value">$${threeDayForecast.values[2].toFixed(2)}</p>
                        <p class="prediction-change ${changePercents[2] >= 0 ? 'text-success' : 'text-danger'}">
                            <i class="fas fa-${changePercents[2] >= 0 ? 'arrow-up' : 'arrow-down'}"></i> 
                            ${Math.abs(changePercents[2])}%
                        </p>
                        <p class="small">
                            <span class="text-muted">Range:</span> 
                            $${threeDayForecast.lower[2].toFixed(2)} - $${threeDayForecast.upper[2].toFixed(2)}
                        </p>
                    </div>
                </div>
            </div>
...
function calculateRSI(period) {
    ...
    document.getElementById('rsi-value').textContent = lastRSI ? lastRSI.toFixed(2) : '--';
...
function createVolumeChart() {
    ...
    if (!columns.includes('Volume')) {
        document.getElementById('volume-plot').innerHTML = 'Volume data not available';
        return;
    }
...
function updateChartWithIndicators(showMA, maPeriod, showBollinger, bollingerPeriod) {
    const dates = stockData.map(row => new Date(row.Date));
    const prices = stockData.map(row => row[selectedColumn]);
    ...
    if (showBollinger) {
        ...
        if (lastPrice && lastUpper && lastLower) {
            const percentPosition = ((lastPrice - lastLower) / (lastUpper - lastLower) * 100).toFixed(1);
...
function calculateSupportResistance() {
    ...
    // Update the DOM
    document.getElementById('r1').querySelector('span').textContent = r1.toFixed(2);
    document.getElementById('r2').querySelector('span').textContent = r2.toFixed(2);
    document.getElementById('r3').querySelector('span').textContent = r3.toFixed(2);
    
    document.getElementById('s1').querySelector('span').textContent = s1.toFixed(2);
    document.getElementById('s2').querySelector('span').textContent = s2.toFixed(2);
    document.getElementById('s3').querySelector('span').textContent = s3.toFixed(2);
}
...
function createTechnicalSummary(rsiPeriod) {
    ...
    const macdSignalLine = calculateEMA(macdLine.filter(val => val !== null), 9);
    
    // Pad the signal line with nulls to match the length of MACD line
    const paddedSignalLine = Array(macdLine.length - macdSignalLine.length).fill(null).concat(macdSignalLine);
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p>Current Value: <strong>${rsiValue ? rsiValue.toFixed(2) : '--'}</strong></p>
                        <div class="indicator-explanation">
                            ...
                            <p>Current Signal: <span class="badge ${rsiValue > 70 ? 'bg-danger' : rsiValue < 30 ? 'bg-success' : 'bg-secondary'}">${rsiValue > 70 ? 'OVERBOUGHT' : rsiValue < 30 ? 'OVERSOLD' : 'NEUTRAL'}</span></p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <h6>MACD Analysis</h6>
                        <p>Current Value: <strong>${lastMACD ? lastMACD.toFixed(2) : '--'}</strong></p>
                        <div class="indicator-explanation">
                            ...
                            <p>Signal: <span class="badge ${lastMACD > 0 && lastHistogram > 0 ? 'bg-success' : lastMACD < 0 && lastHistogram < 0 ? 'bg-danger' : 'bg-secondary'}">${lastMACD > 0 && lastHistogram > 0 ? 'BULLISH' : lastMACD < 0 && lastHistogram < 0 ? 'BEARISH' : 'NEUTRAL'}</span></p>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <p>Position within bands: <strong>${document.getElementById('bb-value').textContent}</strong></p>
                        <div class="indicator-explanation">
                            ...
                            <p>Current Signal: <span class="badge ${bbSignal === 'BUY' ? 'bg-success' : bbSignal === 'SELL' ? 'bg-danger' : 'bg-secondary'}">${bbSignal === 'BUY' ? 'BULLISH' : bbSignal === 'SELL' ? 'BEARISH' : 'NEUTRAL'}</span></p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <h6>Support & Resistance</h6>
                        <p>Current Price: <strong>$${last.toFixed(2)}</strong></p>
                        <div class="indicator-explanation">
                            ...
                            <p>Nearest Resistance: <strong>$${document.getElementById('r1').querySelector('span').textContent}</strong></p>
                            <p>Nearest Support: <strong>$${document.getElementById('s1').querySelector('span').textContent}</strong></p>
                            <p>Price is currently <strong>${last > parseFloat(document.getElementById('r1').querySelector('span').textContent) ? 'ABOVE' : last < parseFloat(document.getElementById('s1').querySelector('span').textContent) ? 'BELOW' : 'BETWEEN'}</strong> key levels.</p>
                        </div>
                    </div>
                </div>
                <hr>
                <div class="overall-analysis">
                    ...
                    <div class="progress mb-3" style="height: 30px;">
                        <div class="progress-bar bg-success" role="progressbar" style="width: ${(buySignals/totalSignals*100).toFixed(0)}%;" aria-valuenow="${(buySignals/totalSignals*100).toFixed(0)}" aria-valuemin="0" aria-valuemax="100">Buy: ${(buySignals/totalSignals*100).toFixed(0)}%</div>
                        <div class="progress-bar bg-secondary" role="progressbar" style="width: ${((totalSignals-buySignals-sellSignals)/totalSignals*100).toFixed(0)}%;" aria-valuenow="${((totalSignals-buySignals-sellSignals)/totalSignals*100).toFixed(0)}" aria-valuemin="0" aria-valuemax="100">Neutral: ${((totalSignals-buySignals-sellSignals)/totalSignals*100).toFixed(0)}%</div>
                        <div class="progress-bar bg-danger" role="progressbar" style="width: ${(sellSignals/totalSignals*100).toFixed(0)}%;" aria-valuenow="${(sellSignals/totalSignals*100).toFixed(0)}" aria-valuemin="0" aria-valuemax="100">Sell: ${(sellSignals/totalSignals*100).toFixed(0)}%</div>
                    </div>
                    <p>Based on the analysis of multiple technical indicators, the current trading signal is: <span class="badge ${overallSignal.textContent === 'BUY' ? 'bg-success' : overallSignal.textContent === 'SELL' ? 'bg-danger' : 'bg-secondary'} fs-6">${overallSignal.textContent}</span></p>
    ...
    document.getElementById('indicators-section').querySelector('.card-body').appendChild(techAnalysisSection);
}
...
function updateDataSummary(data, columns) {
    ...
    const avg = (closes.reduce((sum, val) => sum + val, 0) / closes.length).toFixed(2);
    ...
    dataSummary.innerHTML = `
        <div>
            ...
            <span class="${changeClass}">${changeIcon} ${Math.abs(change)}%</span>
        </div>
    `;
}
