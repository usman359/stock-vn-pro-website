...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (newsTickerSelect)
    newsTickerSelect.addEventListener("change", updateTickerSelection);
  if (useCustomTickerBtn)
    useCustomTickerBtn.addEventListener("click", applyCustomTicker);
  ...
  if (tryDifferentFiltersBtn)
    tryDifferentFiltersBtn.addEventListener("click", showFilters);
...
function updateTickerSelection() {
  ...
  if (newsCompanyElement && newsTickerSelect.selectedIndex >= 0) {
    newsCompanyElement.textContent =
      newsTickerSelect.options[newsTickerSelect.selectedIndex].text;
  }
  ...
  if (rtCompanyNameElement && newsTickerSelect.selectedIndex >= 0) {
    rtCompanyNameElement.textContent =
      newsTickerSelect.options[newsTickerSelect.selectedIndex].text;
  }
...
async function fetchNewsArticles() {
  ...
  // Get ticker
  const ticker =
    currentTicker || (newsTickerSelect ? newsTickerSelect.value : "");
  if (!ticker) {
...
function showNoNewsMessage() {
  ...
  if (newsContainer) {
    ...
    document
      .getElementById("try-different-filters")
...
function generateMockNewsData(ticker, dateRange, category) {
  ...
  try {
    ...
    const seed = Array.from(ticker).reduce(
    ...
    for (let i = 0; i < articleCount; i++) {
      ...
      articleDate.setDate(
        now.getDate() - Math.floor(Math.random() * dateRangeValue)
      );
      ...
      const article = {
        ...
        imageUrl: `https://source.unsplash.com/featured/300x200?business,${
          i + seed
        }`,
        date: articleDate,
        sentiment: sentiment,
...
function generateArticleTitle(ticker, sentiment) {
  const companyName =
    newsTickerSelect.selectedIndex > -1
      ? newsTickerSelect.options[newsTickerSelect.selectedIndex].text.split(
          " ("
        )[0]
      : ticker;
...
function displayNewsPage(page) {
  ...
  loadMoreNewsBtn.style.display =
    endIndex < newsData.length ? "inline-block" : "none";
...
function createArticleElement(article) {
  ...
  const mockNewsUrl = `https://example.com/financial-news/${
    article.id
  }?title=${articleTitle}&source=${encodeURIComponent(article.source)}`;
    ...
    const articleHTML = `
                ...
                <div class="article-container">
                    <div class="article-header">
                        ...
                        <div class="article-meta">
                            <span class="source"><i class="fas fa-newspaper me-1"></i> ${
                              article.source
                            }</span>
      ...
      article.title
                    ...
                    <div class="article-content">
                        ...
                        <div class="article-sentiment sentiment-${
                          article.sentiment
                        }">
  ...
  try {
    ...
    newsData.forEach((article) => {
      if (
        article &&
        article.sentiment &&
        sentimentCounts.hasOwnProperty(article.sentiment)
      ) {
...
function startRealTimeUpdates(ticker) {
  ...
  const realTimeContainer = document.getElementById("real-time-updates");
  if (realTimeContainer) {
    realTimeContainer.style.display = "block";
    realTimeContainer.innerHTML = `
      <div class="card-header">
        <h3><i class="fas fa-chart-line me-2"></i>Real-Time Price Updates</h3>
      </div>
      <div class="card-body">
        <div class="spinner-container">
          ...
          <p class="mt-3 text-center">Loading market data...</p>
        ...
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between mt-3">
            ...
            <div class="real-time-stat">
              ...
              <span class="stat-value" id="rt-market-cap">$0.00B</span>
            </div>
          </div>

          <div class="alert alert-info mt-3 small">
            <i class="fas fa-info-circle me-2"></i>
            Price updates every minute during market hours. Last updated: <span id="rt-last-updated">-</span>
          </div>
        </div>
      `;
